...
export default function ProductForm({ initialData, isEditing = false }: ProductFormProps) {
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      data.append('specifications', JSON.stringify(specifications.filter(spec => spec.label && spec.value)));
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-slate-500">
              {isEditing ? `Modifying: ${initialData?.name}` : 'Add a precision instrument to your catalog'}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="rounded-3xl bg-white p-8 shadow-sm border border-slate-100">
              <h2 className="mb-6 text-lg font-bold text-slate-900">Basic Information</h2>
              ...
              {imagePreviews.length > 0 ? (
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
                  {imagePreviews.map((preview, index) => (
                    <div key={index} className="group relative aspect-square overflow-hidden rounded-2xl border border-slate-100 bg-slate-50">
                ...
                >
                  ...
                  <p className="text-sm font-bold text-slate-500 text-center">
                    Click to upload product images<br/>
                    <span className="text-xs font-medium text-slate-400">Support JPG, PNG, WEBP (Max 5 images)</span>
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="space-y-6">
            {/* Organization Card */}
            <div className="rounded-3xl bg-white p-8 shadow-sm border border-slate-100">
              <h2 className="mb-6 text-lg font-bold text-slate-900">Organization</h2>
              <div className="space-y-6">
                <CustomSelect
                  label="Category"
                  options={categories}
            ...
            {/* Warranty Card */}
            <div className="rounded-3xl bg-white p-8 shadow-sm border border-slate-100">
              <h2 className="mb-6 text-lg font-bold text-slate-900">Warranty Details</h2>
              <div className="space-y-5">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground/80 ml-1">
            ...
            <div className="rounded-3xl bg-slate-900 p-8 text-white shadow-xl shadow-slate-900/10">
              ...
              <p className="text-xs font-medium text-slate-400 leading-relaxed">
                Adding detailed specifications helps customers make informed decisions and reduces return rates for precision electronics.
              </p>
            </div>
          </div>
        </div>
